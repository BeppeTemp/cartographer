...
# a skill belonging to the WORK perimeter, because the provider chooses a skill
...
WS_WORK="${DIR}/work/work-app"
WS_HOMELAB="${DIR}/work/homelab-infra"
WS_UNBOUND="${DIR}/work/scratch"

mkdir -p "$DIR" "$SANDBOX" "$WS_WORK" "$WS_HOMELAB" "$WS_UNBOUND"
kb_make "$WORK_KB"
kb_make "$HOMELAB_KB"
...
E2E_CONFIG="$CONFIG" server_start "${WORK_KB},${HOMELAB_KB}"
...
run_client "${DIR}/connect.log" connect claude --server-url "$SERVER_URL" --kb work-kb --auto-trust || true
assert_file_exists "${SANDBOX}/.claude.json"

# The global catalogue holds the WORK skill, reachable from every directory on
...
assert_file_contains "${SANDBOX}/.claude/skills/new-microservice/SKILL.md" "PERIMETER-WORK"

if run_client "${DIR}/bind-work.log" workspace bind claude "$WS_WORK" --kb work-kb; then
    _assert_pass "workspace bind claude <work workspace> --kb work-kb"
else
    _assert_fail "workspace bind failed: $(cat "${DIR}/bind-work.log")"
fi
assert_file_contains "${DIR}/bind-work.log" "workspace scope"
...
WORK_SKILL="${WS_WORK}/.claude/skills/new-microservice/SKILL.md"
HOMELAB_SKILL="${WS_HOMELAB}/.claude/skills/new-microservice/SKILL.md"

assert_file_exists "$WORK_SKILL"
assert_file_exists "$HOMELAB_SKILL"
assert_file_contains "$WORK_SKILL" "PERIMETER-WORK"
assert_file_not_contains "$WORK_SKILL" "PERIMETER-HOMELAB"
assert_file_contains "$HOMELAB_SKILL" "PERIMETER-HOMELAB"
assert_file_not_contains "$HOMELAB_SKILL" "PERIMETER-WORK"
...
assert_file_not_exists "${WS_WORK}/.claude/skills/cartographer-ops/SKILL.md"
...
for ws in "$WS_WORK" "$WS_HOMELAB"; do
...
CANON_WORK="$(cd "$WS_WORK" && pwd -P)"
CANON_HOMELAB="$(cd "$WS_HOMELAB" && pwd -P)"

run_client "${DIR}/list.log" workspace list
assert_file_contains "${DIR}/list.log" "$CANON_WORK"
assert_file_contains "${DIR}/list.log" "$CANON_HOMELAB"
assert_file_contains "${DIR}/list.log" "work-kb"
assert_file_contains "${DIR}/list.log" "homelab-kb"

run_client "${DIR}/status.log" status || true
assert_file_contains "${DIR}/status.log" "workspace ${CANON_WORK}"

# Unbinding one workspace prunes only its files.
if run_client "${DIR}/unbind.log" workspace unbind claude "$WS_WORK"; then
    _assert_pass "workspace unbind claude <work workspace>"
else
...
assert_file_not_exists "$WORK_SKILL"
assert_file_exists "$HOMELAB_SKILL"
assert_file_contains "$HOMELAB_SKILL" "PERIMETER-HOMELAB"
...
run_client "${DIR}/bind-both.log" workspace bind claude "$WS_UNBOUND" --kb work-kb,homelab-kb
