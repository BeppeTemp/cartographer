...
| `index_get(path, [with_hash])` **[R]** | Reads a folder's `index.md` (progressive disclosure). By default returns raw Markdown verbatim, byte-for-byte, unchanged for any existing caller. `with_hash: true` instead returns structured `{path, content, content_hash}`; `content_hash` is the `if_match` `index_patch` expects (D122). |
