...
**Read freshness across instances (D93)**: when `git.sync` is enabled and a KB has an `origin`, every tool marked **[R]** that finds the `git.in_window` expired starts a fetch + pull-rebase of that KB in the background and answers at once from the local clone (D258): a remote change is visible from the next call on, one call late, and no read waits on the remote. At most one such refresh runs per KB. A pull that moves HEAD reconciles the derived indexes, so a remote concept becomes both readable and searchable. Read-side sync is best-effort: a fetch error is logged; a rebase conflict is registered and its concepts are marked degraded. There is no periodic poller: a KB nobody reads is not fetched. Write tools still fetch and rebase before the write, synchronously (D237).

## MCP API

This list is the **source of truth** for the active tools (do not duplicate counts elsewhere).

Tools marked **[R]** have `Tool.ReadOnly=true` (`internal/mcpserver`): they never mutate KB content and remain callable with a `kb:<name>:r` scope. All others require `rw`. Enforcement and scope format → [`transport-auth.md`](transport-auth.md) §Per-KB authorization.
...
| Tool | Purpose |
|---|---|
| `atlas_overview([structure])` **[R]** | Root index + maps/journals (concept count, and expanded concept count if any). `structure: true` appends a `## Structure` section computed on the concepts the caller can see (D244): `### Most central`, the top 10 by PageRank, and `### Communities`, up to 12 communities of at least 3 concepts, each named after its anchor, with its size and dominant map. Without it the response is unchanged. |
...
| `changes_since([since], [limit])` **[R]** | Git-history digest since an RFC3339 timestamp or `<N>d`/`<N>h` duration (default 7d). Returns each changed concept's newest status (`added`, `modified`, `deleted`, `moved`), latest timestamp, authors and recent operations; map/log/artifact changes are counted as `other_changes`. Results default to 100 concepts (maximum 500) and report `truncated`; no commits returns a JSON note rather than an empty string (D94). `links: true` adds `links` {`added`, `removed` [{source, target, target_missing?}], `became_orphan`, `no_longer_orphan`, `truncated?`}: the link changes of the concept files changed in the range, their links at the parent of the oldest commit against their links now, only between concepts the caller can see; at most 500 edges, and past 1,000 changed concept files `links` is omitted with a `links_note` (D250). |
| `graph_neighbors(id, [depth], [direction])` **[R]** | Graph neighbors (used for lint scoping). `direction` is `out` (default, links from the page), `in` (backlinks to it), or `both`; traversal returns minimum distances and can start from a missing target for backlink inspection. The graph sees both markdown links `[text](rel.md)` (relative to the file) and wiki-links `[[id]]`/`[[id#section]]` (root-relative, D72 WP0). Ask `direction: "in"` before editing a runbook to see which pages cite it. A neighbour that is a link target but not a concept carries `"missing": true` (D241); a real one has no such field. Any `depth` is allowed to every principal: the walk runs on the concepts the caller can see, so a hidden concept is never a result, a transit node or a `missing` target (D249); a narrowed token asking for an id that does not exist gets `not found`. |
| `graph_context([query], [ids], [limit])` **[R]** | Ranked context around a question or up to 10 concepts (D242). Seeds are the top 5 `search` hits for `query`, weighted by reciprocal rank, plus each of `ids`; the ranking is personalized PageRank (restart 0.25) over links in both directions. Returns `seeds` and `results` [{id, title, score, hops, via, snippet}] — `via` is the concept a result was reached through, `snippet` present only with a `query` — sorted by score, then hops, then id; seeds are not results. `limit` defaults to 10 (1–30). A query with no hit and no `ids` returns an empty list with a `note`. Ask it instead of chaining `search`, `concept_read` and `graph_neighbors` one hop at a time. |
| `link_suggest(id, [limit])` **[R]** | Concepts `id` probably should link to (D242): those sharing at least two neighbours with it and not linked to it in either direction, scored by resource allocation (Σ 1/degree of each shared neighbour, so evidence through a hub counts less). Deprecated and superseded candidates are left out. Returns `candidates` [{id, title, score, common}] with up to 5 shared neighbours each; `limit` defaults to 5 (1–20). Ask it after creating a concept. |
| `graph_path(source, target, [direction], [max_hops])` **[R]** **[A]** | The shortest chain of links from `source` to `target` (D242), either way (`both`, default) or as written (`out`), within `max_hops` (default 6, 1–12); among equal-length chains, the one first in id order. Returns `path` [{id, title}] and `hops`, or an empty `path` with a `note`. |
...
| Tool | Purpose |
|---|---|
| `search(query, [scope], [limit])` **[R]** | Keyword search — the only mode (D135): `mode` and `use_semantic` are no longer accepted and a call passing either is rejected. Keyword matching first requires all query terms, then retries with any term if that returns no hits. Every hit includes `title` (from the frontmatter) and `snippet` (an excerpt of ~200 chars around the match; FTS5 uses its native `snippet()`, otherwise it's extracted in-memory) — avoids a `concept_read` just to judge a hit's relevance (D70). `scope` is a **literal** id prefix on both backends: `_` and `%` match themselves, not the SQL `LIKE` wildcards they would otherwise be. Among near-equal matches, the concept the KB links to most ranks first: `score` includes a centrality prior of at most +20% (D251, §Search index). |
| `reindex([full])` **[A]** | Runs the same reconciliation every search runs (D245) and reports its counts — `indexed`, `updated`, `removed` — with or without a SQLite index. Search is already fresh without it; it is an explicit check after imports or manual edits. With `full: true` it rebuilds the whole index from every concept instead, returning `status: "rebuilt"`, `concepts_indexed` and — only with a SQLite index — `sql_upserted`. It requires write scope because it writes the server-owned SQLite index (D90, D136). |
...
| `asset_list(concept_id)` **[R]** | Lists an expanded concept's non-Markdown regular assets with path, size, raw-byte `sha256`, and executable mode. Hidden files are skipped; a file above the cap is listed with `oversized: true` (D270). |
| `asset_write(concept_id, path, content, [encoding], [executable], [if_match])` | Creates or updates an expanded concept asset. 10 MiB decoded limit (the largest file worth versioning in git, D270); `.md`, hidden, escaping and symlink paths are rejected. `if_match` is forbidden on create and mandatory on overwrite; omitted `executable` defaults false on create and preserves mode on overwrite. |
| `asset_delete(concept_id, path, if_match)` **[A]** | Deletes an asset with mandatory raw-byte `if_match` — an oversized one included — pruning empty asset-only directories but never the owner directory. |
...
| `concept_patch(id, old_string, new_string, [replace_all], if_match, [frontmatter])` | String-replace patch on the body only (Edit-like semantics), without rewriting the entire concept. `if_match` is **mandatory**: fails with `stale_write` if changed. Fails with `old_string_not_found` or `old_string_ambiguous` (use `replace_all` for multiple matches). `frontmatter`, if present, is shallow-merged onto the existing frontmatter, and may be the **only** change — no body edit, with `edits` absent or empty, e.g. to set a missing `title` (#321); a key set to `null` is removed rather than set to a literal null (fails if the key is required, e.g. `type`, D88). Same write path (indices, commit) as `concept_write` (D70). As an alternative to the `old_string`/`new_string`/`replace_all` triple, it accepts an `edits: [{old_string, new_string, replace_all?}]` field to apply several patches in a single call (one commit): the two forms are mutually exclusive; edits are applied **in order, atomically** (edit i+1 sees the result of edit i) — if one fails, nothing is written and the error reports the index of the failed edit (D76). |
...
| `supersede(source_id, target_id, [reason])` | Marks a concept as superseded by another: sets `status: superseded` and `superseded_by`, and updates both search indexes at once. The successor must be an existing concept other than the source (`supersede: target not found: <id>`, `supersede: a concept cannot supersede itself`); `superseded_by` is a relation lint checks, not a graph edge (D243). |
| `concept_move(source_id, target_id \| moves[], [rewrite_links])` | Moves one or more concepts (batch `moves: [{source_id, target_id}]`, single form kept for backward compatibility; the two forms cannot be mixed). Validates the entire batch before applying any move (application-level atomicity), one git commit per call. With `rewrite_links` (default `true`) it rewrites backlinks — wiki-links `[[old]]`/`[[old#section]]` and relative markdown links, never those inside code spans — in every concept the link graph says links to a moved id, reading only those (D248), using the old→new map, and updates the indices (in-memory + FTS5) for both the moved and rewritten concepts; the result lists the applied moves and rewritten concepts. The same pass rewrites a `superseded_by` that names a moved concept (D243). With `rewrite_links=false` backlinks and relations are left intact (warning in the result, use `lint`). Both ends are resolved in their own namespace — a map under `data/` or the KB-root `services/` — so a move works in any direction between them: on success the old ID is not found and the new one is readable, with no copy left behind (D269). An occupied destination (a concept, an unparsable file, or an `<id>/` directory holding only assets) is a conflict found before anything moves. A filesystem failure after a target was written (source removal, or an expanded directory's rename) is an application error naming both IDs: no commit, no success log entry, no rollback. D72 WP1/WP2, D248, D269. |
| `concept_batch(operations)` **[A]** | Atomically writes/patches several **distinct** concepts as one logical operation: one git commit, one summary `log.md` entry, either every operation applies or none does (D125). Each entry is `write` (frontmatter, body, optional `if_match` — absent means create-only, **required** to update an existing concept) or `patch` (required `if_match`, optional frontmatter shallow-merge — which may be the only change — and the same single/`edits` Edit-tool semantics as `concept_patch`). Bounded: capped operation count and aggregate content size, duplicate/invalid IDs rejected; every operation — including each Map's strict-ontology palette and required-field contract — is validated before anything is written. Delete, move, expand, assets, and Map/root curated indexes stay out of scope: use `concept_delete`/`concept_move`/`concept_expand`/`asset_*`/`index_patch` for those. For confined single-concept edits use `concept_patch`'s own `edits` batch (D76) instead; for renames use `concept_move` (D72); this tool is for a multi-page refactor that touches several concepts at once, where separate `concept_write`/`concept_patch` calls would leave partially-aligned intermediate commits if interrupted. |
...
| `lint([scope], [scope_neighbors], [severity_min])` **[R]** | Runs deterministic broken-link, stale-claim, orphan, contract and structural checks. The graph-shape checks (D243) are `island` (a group of 2+ concepts linked to each other and to nothing else), `cut_concept` (a concept whose removal disconnects 3+ others), `link_to_retired` (a live map concept linking a deprecated or superseded one, naming its successor), `map_misfit` (2/3 of 4+ map neighbours in one other map) — all `info` — and `broken_relation` (`warning`: a `superseded_by` that names no concept, or the concept itself). They are computed on the whole KB and reported only for concepts in `scope`, so a scoped lint gives the same verdict. `scope_neighbors=true` adds one-hop graph neighbors. There is no model-backed/deep mode. `severity_min` (info/warning/error) floors the returned findings — default `info`, exhaustive, because `lint` is the tool an operator calls *to see* findings (D186). `count` is always the unfiltered total, and `counts_by_check`/`counts_by_severity` are computed before filtering, so a caller always knows the shape of what it is not being shown. |
...
| `kb_status()` **[R]** | The `kb` field names the KB that served the call (D144). Aggregate metrics: total concepts, per type, per `status` (concepts with no `status` field are excluded, D131), stale ones, open contradictions. Plus the replication facts an agent needs without reaching for `sync_status` (D145): `has_remote` / `remote_url` (`origin`, credentials redacted), `git_sync`, `push_state`, `push_last_error`, `unpushed_commits` (`null` when no trustworthy remote-tracking comparison exists), and the write workflow `git_workflow` (`local` \| `server`, D117 — it says nothing about remote presence). Read-only: no fetch, no network. `git_profile` is a deprecated alias of `git_workflow`, kept for one minor release. `search_misses` (D247) lists the 10 most frequent queries of the last 30 days that found nothing — `query` (normalised), `count`, `last_seen` — sorted by count, then last seen; omitted when there are none. `server_version` is the serving binary's version, and `latest_version` a newer release the server's own daily update check knows of (D254) — omitted when none is known; reading it makes no network call. |
...
| `sync_pull()` **[R]** **[A]** | Read-only, no parameters. Returns base64 file contents plus optional detached Ed25519 signature (`algorithm`, `key_id`, envelope version, value). Remote clients recompute hashes and verify pinned keys before materialization. KB-provided MCP descriptors appear only when the server-side allow-list permits them. Also returns `placeholders` (omitted when empty): the sorted `kind:key` path placeholders (D75) the KB cites — in the bodies of the concepts the caller may see (projected like the graph tools, so a key only a hidden concept cites is never listed) and in its own artifacts — for the client to resolve (D262). Also returns `path_registry` (omitted when the KB has no `paths.yaml`): the KB's declared placeholder keys, `{"paths": {key: {description, default}}, "repos": {key: {description, remote, default}}}`, with malformed entries left out and named in `issues` (D263). `issues` (omitted when empty) also carries the manifest's reasons for leaving an artifact out. None of the three is part of `revision` or signed; the server still expands nothing. |
| `sync_status()` **[R]** **[A]** | Read-only. Returns local Git replication state (`disabled`, `no_remote`, `clean`, `pending`, `failed`, or `degraded` when writes are blocked), the last error/attempt, HEAD, best-effort unpushed count, identity warning, the checked-out `branch` and the `remote_default_branch` last resolved from the remote (a mismatch blocks local-profile writes, D264), push mode, and the write workflow `git_workflow` (D145; `profile` is its deprecated alias, kept for one minor release). |
...
| Tool | Purpose |
|---|---|
| `artifact_read(path, [encoding])` **[R]** | Reads a KB-root artifact file (`skills/<slug>/**`, `agents/<slug>.md`, `hooks/**`, `mcp/<slug>.json`, `instructions.md`, `templates/<slug>.md`, `paths.yaml`) and returns its content, `encoding` (`text` or `base64`) and raw-byte `sha256` (the `if_match` for `artifact_write`). Non-UTF-8 bytes are always base64. |
...
| `artifact_write(path, content, [encoding], [executable], [if_match])` | Creates/updates a KB-root artifact file. `encoding` is `text` (default) or `base64`; the 256 KiB limit applies after decoding. `executable` is tri-state: omitted preserves an existing mode and defaults false on creation. Structured principal files remain UTF-8 and non-executable. Templates require parseable frontmatter with a literal non-empty `type` and only `{{identifier}}` variables in scalar/list values or body. `paths.yaml` (the KB's path placeholder registry, D263) is validated strictly: only `paths:`/`repos:`, slug keys, a required `description`, a `default` starting with `~/` or `$HOME/`, a parseable `remote` on repo keys — any other field or value is refused with the entry named. On an existing file, `if_match` (raw-byte sha256) is **mandatory** (`already_exists` if missing, `stale_write` if wrong); per-kind validation applies before the write. Only registered if the KB has `allow_artifact_write: true`. This is the write end of what a materialized skill or agent advertises: their provenance block ([D138](decisions/D138-provenance-stamp-on-materialized-skills-and-agents-and.md)) names this tool, the KB and the path to call it with. |
...
|---|---|---|
| 10 MiB per asset | `kb.AssetMaxFileSize` (`internal/kb/asset.go`) | `asset_write`, `asset_read` |
...
| `GET /api/ui/v1/kbs/{kb}/graph?scope=&limit=` | A bounded graph snapshot: sorted nodes with their collection, `title` (when the concept has one), `type`, `status`, both degrees, `pagerank` and `community`, directed deduplicated edges, broken link targets kept apart from the nodes, and `communities` [{rank, size, anchor, slot}]. PageRank and communities are computed on the whole graph the caller can see, before scope and limit, so a scoped view keeps whole-KB colours (D244). `scope` is one top-level collection; `limit` defaults to 2,000 nodes and is clamped to 5,000, with the effective value echoed back. Past the limit the nodes with the highest PageRank are kept, and past 20,000 edges those whose weaker endpoint ranks highest; the output stays sorted. |
...
| `GET /api/ui/v1/kbs/{kb}/lint?scope=&severity_min=` | Findings at or above the floor, plus the unfiltered totals by severity and by check — the same "counts describe what you are not being shown" contract `lint` has. A caller that cannot see the whole KB gets no `island`, `cut_concept`, `link_to_retired` or `map_misfit` finding: they are computed on the whole graph and would disclose hidden concepts (D243). |
...
- **In-memory (default/Core)**: a pure-Go keyword inverted index (`internal/search`). Rebuilt on every startup by walking the concepts. Tokens are lowercased and diacritic-folded ("attivita" finds "attività"; ß, æ, œ stay as they are), and weighted like the SQLite columns: title tokens count 5×, frontmatter tokens 2×, body tokens once (D246). Snippets are cut from the text as written, accents included.
- **Centrality prior (D251)**: both backends' text scores are adjusted the same way before the final sort and cut: `score = text × (1 + 0.2 · p)`, where `p` is the concept's PageRank percentile among the concepts the caller can see (the share of the others with a strictly lower PageRank). A concept nothing links to has `p = 0` and keeps its text score exactly; the boost is at most +20%, enough to reorder near-ties and too small to lift a weak match over a strong one. The backend returns a window of `min(3 × limit, 100)` candidates (never fewer than `limit`) so a central concept just below the page can enter it. For a narrowed principal the percentiles come from its visible graph, so a hidden hub never moves the order; the whole-KB percentiles are cached per link-graph generation. `graph_context` seeds from the same ranking. The reported `score` is the boosted one.
- **Miss log (D247)**: a `search` that returns zero results — after the all-terms → any-term retry — for a principal who can see the **whole** KB is appended to `.cartographer/search-misses.jsonl` (local, never committed): the time and the query, trimmed, lowercased, diacritic-folded and whitespace-collapsed. A narrowed principal's misses are never recorded: for them zero hits may only mean "hidden", and recording would show their queries to whole-KB readers. The file is rewritten to its last 2,000 lines past 2,500; a write failure is logged and never fails the search. `kb_status` reports it as `search_misses`.
- **Freshness by validation (D245)**: no write path updates an index. Before every `search` (and `graph_context`'s seeding), after every git pull and on `reindex`, the link-graph cache (D241) validates the concept files — a stat walk, zero file reads when nothing changed — and reports every concept whose resolved content changed, appeared or vanished since the last reconciliation; one per-KB reconciler applies that delta to the in-memory index and, when present, to SQLite. A write handler, a move, a conflict resolution, a pull or an operator's editor are seen the same way, with or without git and with or without SQLite.
- **Persisted SQLite (`internal/sqlindex`, D32)**: when the KB has an openable `.cartographer/index.db`, keyword search uses **FTS5 with a trigram tokenizer** (supports substrings, not just whole words) and `remove_diacritics 1`. The table has three indexed columns — `title` (frontmatter `title`), `meta` (the raw frontmatter), `body` (the body) — ranked by bm25 with weights 5 / 2 / 1, so a concept named after the query beats pages that mention it in passing (D246). The layout is versioned with `PRAGMA user_version`: opening an older database drops the FTS table and empties `concepts`, and the next reconciliation reindexes every concept — once, automatically. Multi-term matching tries all terms first, then any term only if the all-terms search is empty; terms shorter than three characters are omitted from the FTS match. At boot the persisted hashes are reconciled with the files (this also drops rows for concepts removed while the server was down); while running, the D245 reconciler keeps the table in line, skipping an upsert whose hash SQLite already stores; `reindex(full: true)` rebuilds every concept instead of diffing (D136). On this path, the `search` response's `mode` field is `keyword_fts5`. A database created before D135 may still carry an `embeddings` table: it is never read nor written, and needs no migration. Best-effort: if the DB can't be opened or FTS5 is unavailable, it degrades to the in-memory path.
...
- When the KB has a `paths.yaml` (D263), a concept citing an undeclared
  placeholder key is `unknown_placeholder` (warning), a declared key nothing
  cites is `unused_placeholder` (info), and a malformed entry is
  `contract_malformed` (info) on `paths.yaml`; see `data-plane.md`.
