...
- **Persisted SQLite (`internal/sqlindex`, D32)**: when the KB has an openable `.cartographer/index.db`, keyword search uses **FTS5 with a trigram tokenizer** (supports substrings, not just whole words). Multi-term matching tries all terms first, then any term only if the all-terms search is empty; terms shorter than three characters are omitted from the FTS match. At boot the persisted hashes are reconciled with the files (this also drops rows for concepts removed while the server was down); while running, the D245 reconciler keeps the table in line, skipping an upsert whose hash SQLite already stores; `reindex(full: true)` rebuilds every concept instead of diffing (D136). On this path, the `search` response's `mode` field is `keyword_fts5`. A database created before D135 may still carry an `embeddings` table: it is never read nor written, and needs no migration. Best-effort: if the DB can't be opened or FTS5 is unavailable, it degrades to the in-memory path.
