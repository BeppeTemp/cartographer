...
**The winget channel** ([D218](decisions/D218-winget-is-the-only-windows-channel-and-the-archive-is-a-zip.md)). The darwin and linux assets stay raw per-platform binaries, because that is what `install.sh` downloads and verifies against its line in `sha256sums.txt`; the two Windows assets are **zips** (`cartographer-windows-{amd64,arm64}.zip`, each containing `cartographer.exe`). The format is what the winget pipe reads: a zip yields `InstallerType: zip` with `NestedInstallerType: portable` and a `PortableCommandAlias` of `cartographer`, while a raw binary would register the command as `cartographer.exe`. GoReleaser writes the three manifests (version, installer, locale) for `BeppeTemp.Cartographer`, pushes them to the fork `BeppeTemp/winget-pkgs` on a branch `cartographer-<version>`, and opens a pull request against `microsoft/winget-pkgs`.

Maintainer prerequisites, once, **before the first tag carrying Windows assets**: fork `microsoft/winget-pkgs` as `BeppeTemp/winget-pkgs`, create a PAT that can push to the fork and open pull requests, and add it as the `WINGET_PKGS_TOKEN` repository secret. A missing token does **not** fail the release — the winget pipe continues on error — it simply publishes no manifest, which is why this is a prerequisite and not a blocker. Every subsequent tag opens another pull request, and the first submission goes through Microsoft's automated validation and human review, so expect it to need attention.

Secrets required in the GitHub repo: `RELEASE_PLEASE_TOKEN`, `HOMEBREW_TAP_TOKEN`, `WINGET_PKGS_TOKEN`. `GITHUB_TOKEN` is provided automatically by Actions (for ghcr.io the `packages: write` permission declared in the workflow is enough).
...
One channel per platform: `brew install beppetemp/tap/cartographer` on macOS (cask from the tap), `winget install BeppeTemp.Cartographer` on Windows, and `install.sh` (repo root) on Linux or on a macOS without Homebrew. The script downloads the latest `cartographer` binary from the most recent GitHub Release for the current platform:
...
**`install.sh` is Darwin and Linux only, and says so where it matters.** Run from Git Bash, MSYS2 or Cygwin — where `uname -s` reports `MINGW64_NT-…`, `MSYS_NT-…` or `CYGWIN_NT-…` — it refuses with the winget command instead of a generic "unsupported OS", because winget is the only Windows channel and that string is the whole remedy. It also warns there that `cartographer service uninstall` has to come before removing the package: `winget uninstall` runs no Cartographer code, so a registered Scheduled Task would outlive the binary it points at. That is the Windows counterpart of the script's own refusal to uninstall a binary while the native units are still installed — which cannot exist on Windows, since nothing of ours runs at uninstall time. `winget upgrade` is the same lazy-repair story as the Homebrew Cask (D199): the next `cartographer sync` replaces a service still running the previous binary.
