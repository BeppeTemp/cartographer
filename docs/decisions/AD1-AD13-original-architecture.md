...
This is the one file that holds more than a single decision, and deliberately:
the thirteen ADs never had individual bodies — they are one historical table,
written as one, every row already superseded by the current documentation.
Splitting them into thirteen files each holding one table row would add
navigation without adding anything to read.
