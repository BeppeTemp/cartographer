...
A KB can distribute third-party MCP servers (endpoints that connected agents must be able to call) alongside skills/agents/hooks/instructions. Only the **http** transport is supported: any other `type` fails the build (issue #55).
...
- **No secrets in the file**: `headers` and `env` values only support `${VAR}` references, resolved by the client against its own environment at materialization time — the same pattern as `token_env` (D64). `env` is validated but no provider emits it: none of the 4 exposes a verified native channel for generic env vars on an "http" server — only the `Authorization: Bearer ${VAR}` header can be reliably represented (issue #55).
