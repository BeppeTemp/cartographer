...
[synchronization/provisioning decisions](decisions.md#sync-provisioning) and
[client decisions](decisions.md#client-configurator).
...
Three optional per-provider fields, all empty for everything written before they existed and all meaning "unchanged behaviour" when absent: `base_dir` — the directory `managed[].path` is relative to, recorded only for a provider that materializes outside the shared base dir ([D141](decisions/D141-hermes-is-a-supported-provider-that-receives.md)); `server_version` — the Cartographer server version this state was materialized against ([D142](decisions/D142-reconnect-rebuild-a-client-configuration-never.md)), which `sync` compares with the live one to report that the server changed, and which a sync that cannot reach `/health` leaves untouched rather than erasing; `executable_paths` — the artifact-relative paths this artifact's files were written executable at, which is what lets a client whose filesystem has no execute bit recompute `materialized_hash` at all (see §Executable flag).
...
Hook **`cartographer-bootstrap`** (reserved name, `provisioning.BootstrapHookName`): generated entirely by the client (never from the manifest), installed/updated by `EnsureBootstrapHook` on every `connect`/`sync` and removed by `disconnect`. At the start of a session, the generated script (deterministic, silent, always `exit 0`) checks that `cartographer` is resolvable and launches `cartographer sync --auto-trust` in the background. The script is the platform's own: `bootstrap.sh` (`#!/bin/sh`, `command -v`) on unix, `bootstrap.cmd` (batch, `where`) on Windows, where a shell script is not executable in any sense the operating system recognises and the explicit `chmod` that follows the write grants nothing ([D216](decisions/D216-the-client-half-reaches-parity-on-windows.md)). Explicitly excluded from `ComputeDiff`/`Apply` (a server manifest will never contain it → it's never treated as an orphan to prune). Details → [D60](decisions/D60-client-side-bootstrap-hook-auto-sync-at-session-start.md).

After the silent sync the script runs `cartographer update notice`, keeping its stdout and discarding its stderr ([D254](decisions/D254-agents-are-told-when-cartographer-is-out-of-date.md)). It prints nothing unless a newer release is known, then one paragraph that a client adding SessionStart hook output to the session context (Claude Code) hands to the agent; the rest of the script's guarantees are unchanged. Because the hook is excluded from diffing, nothing compares an installed script with the current one: what delivers a changed script to an already-connected machine is that `EnsureBootstrapHook` rewrites both files **unconditionally** on every `connect`/`sync`, and the hook itself runs `sync` — so the first session start after an upgrade installs the new script and the next one runs it, with no `reconnect`. A test pins that rewrite.

| Provider | Hook point | Registration |
|---|---|---|
| claude | `SessionStart` | entry in `~/.claude/settings.json` |
| codex | `SessionStart` | entry in `~/.codex/hooks.json` (D230) |
| opencode | `session.created` event | generated plugin in `~/.config/opencode/plugins/` |
| kiro | — (its hooks are declared per agent, not per machine, so none fires for the agent the user runs — see [D140](decisions/D140-a-scheduled-sync-trigger-for-clients-with-no-session.md)) | the scheduled trigger below, or Layer 2 |
...
- **Symlinked destinations are refused** (D148). `os.WriteFile` on a symlinked path opens the *target* with `O_WRONLY|O_TRUNC` — it does not replace the link — so a symlinked client directory diverts the write outside the client, and when the target is another git repository it lands there with a provenance footer declaring a false origin. Every materialization walks the destination's components under the base dir with `os.Lstat` and refuses the first one that is not a plain file or a plain directory — a symlink, a device, a socket, a named pipe, or anything Go can only report as `ModeIrregular`, which is one of the two ways a Windows directory junction may appear ([D216](decisions/D216-the-client-half-reaches-parity-on-windows.md): the guard refuses both spellings, because which one Go uses there is unverified). A regular file where a directory is expected stays accepted, since `MkdirAll` already reports that accurately. The base dir itself is exempt, since it may legitimately be a link (a symlinked `$HOME`, a provider root from `BaseDirEnv`). The same widened refusal applies to `internal/sops`' path walk, which has the same shape and the same exposure. The refusal is **per artifact**: it is reported in the sync summary as `refused: <kind>/<name>` and in `AppliedResult.Refused`, the rest of the pass proceeds, and the artifact is left out of the lockfile so the next sync retries and reports again — the condition can only be cleared by the operator. `cartographer doctor` reports a symlinked destination directly, since a sync only mentions it on a run where that artifact is in the diff. Symlinked client-config directories are ordinary (a dotfile manager, a monorepo checkout, a shared team directory, an earlier bootstrap that linked skills out of a source repo), which is why this is a refusal rather than an assumption.
...
- Round-trip `connect`→`disconnect` leaves no residue, except for the provisioning roots themselves (deliberate boundaries): test `TestRoundTrip_ConnectDisconnect_NessunResiduo`. Details → [D63](decisions/D63-complete-prune-empty-directories-with-boundaries-mcp.md).
...
| Kind | Check |
|---|---|
| `skill`, `hook` | the artifact's own directory is re-hashed with the same helper that produced `materialized_hash` — an extra file left inside counts as modified, and a lost executable bit is part of the hash **where the filesystem has one** (see §Executable flag) |
...
| `hook` on claude | also: every `settings.json` command registered for it can run — not a backslash path on Windows, not an absolute path to a file that is not there. One that cannot is `unrunnable`, with the command and the reason in the finding's detail, and the next `sync` re-registers it (D267) |
| `hook` on codex | also: no registration of it left in `.codex/config.toml` — one there is `unregistered`, so the next `sync` rewrites the hook and migrates the registration to `hooks.json` even when its content did not change (D236) |

Findings are `missing`, `modified`, `unregistered`, `unrunnable`, or `unknown`. **Existence and content are
verified separately** ([D146](decisions/D146-existence-and-content-are-verified-with-different.md)), because they need different
...
3. `fetchMergedManifest` → signature verification and cross-KB collision detection ([D171](decisions/D171-a-cross-kb-collision-is-an-error-not-an-alphabetical.md));
...
|---|---|---|---|---|---|---|---|
| `skill` | `.claude/skills/<name>/` | `.opencode/skills/<name>/` | `.codex/skills/<name>/` | `.kiro/skills/<name>/` | `skill-inbox/<name>/cartographer/` (delivered, see below) | `.gemini/config/skills/<name>/` | `.config/crush/skills/<name>/` |
| `agent` | `.claude/agents/<name>.md` (verbatim) | `.opencode/agent/<name>.md` (translated) | `.codex/agents/<name>.toml` (translated) | `.kiro/agents/<name>.json` (translated) | unsupported — no native subagent directory | `.gemini/config/agents/<name>.md` (translated) | unsupported — no documented user-level subagent directory |
| `hook` | `.claude/hooks/<name>/` + registration in `settings.json` | `.opencode/hooks/<name>/` + generated JS plugin | `.codex/hooks/<name>/` + registration in `hooks.json` | unsupported — no hook mechanism in the shipped client (D195) | unsupported — no hook mechanism at all | `.gemini/config/hooks/<name>/` + registration in `hooks.json` | unsupported — no hook mechanism documented |
| `instructions` | managed block in `.claude/CLAUDE.md` | block in `.config/opencode/AGENTS.md` | block in `.codex/AGENTS.md` | file `.kiro/steering/cartographer.md` | unsupported — `SOUL.md` is operator-owned, rendered from a template | block in `.gemini/GEMINI.md` | block in `.config/crush/CRUSH.md` |
| `mcp` | key `mcpServers.<name>` in `.claude.json` | key `mcp.<name>` in `opencode.json` | block `[mcp_servers.<name>]` in `config.toml` | key `mcpServers.<name>` in `.kiro/settings/mcp.json` | unsupported — `config.yaml` is rendered by an Ansible role | key `mcpServers.<name>` in `.gemini/config/mcp_config.json` | key `mcp.<name>` in `.config/crush/crush.json` |
...
single lockfile ([D141](decisions/D141-hermes-is-a-supported-provider-that-receives.md)). An entry with no `base_dir` — every
...
provenance footer is in the body, read only *after* activation. That is how a WORK skill got
...
file on the machine ([D169](decisions/D169-per-provider-kb-binding-a-user-owned-preference-beside.md)). What this prevents is accidental
...
[D189](decisions/D189-instructions-written-correctly-are-not-reported-as.md) exists to eliminate.
...
- **Agent translation** (`translateAgentForProvider`, a pure function): the source is always a Claude subagent. Claude = passthrough; OpenCode = minimal frontmatter `description` + `mode: subagent` + verbatim body; Codex = TOML `name`/`description`/`developer_instructions`; Antigravity = Markdown frontmatter with `name`, `description`, `mainAgent: false`, `subagent: true`; Kiro = a JSON config with `name`/`description`/`prompt` ([D195](decisions/D195-kiro-receives-subagents-its-hooks-are-documented-but.md) — the format the shipped client discovers, which is **not** the Markdown its documentation describes). The body stays verbatim; fields that can't be mapped reliably (`tools`, `model`) are dropped, not guessed. Details → [D55/D58](decisions/D55-agents-also-materialized-on-opencode-via-frontmatter.md).
- **Hook registration**: besides materializing the files, `Apply` registers the hook in the provider's native mechanism (`internal/provisioning/hooksettings.go`), idempotently and prunably:
  ...
  - *codex*: an entry in `.codex/hooks.json`, whose shape is Claude Code's `settings.json` hooks, patched in place by the same `.codex/hooks/<name>/` ownership marker; other integrations' entries (Herdr's, the user's) are preserved, and a `hooks.json` left holding nothing is removed on prune (D230). A registration an older client wrote into `.codex/config.toml` — the `# cartographer:hook:<name>:begin/end` block, and any marker-less copy Codex's own rewrite left outside it (D99) — is migrated out on the next `Apply` and reported in `AppliedResult.Warnings`: Codex keys its hook trust by file, so it asks to trust the moved hook once. An unchanged hook is migrated too, through on-disk verification (D236), and any table Codex placed inside the old block that is not one of the hook's `[[hooks.<Event>]]` registrations (a `[notice]`, trust bookkeeping) is moved out of it before the block is deleted. `doctor` counts a registration still in `config.toml` as a stray, since it fires alongside the `hooks.json` one;
  ...
  - a missing/malformed `hook.json` skips registration without failing `Apply`; a `command` whose first token is a relative path — one containing either separator, e.g. `./notify.sh` or `scripts\run.sh` — is resolved to the materialized absolute path, and quoted with double quotes if that path contains whitespace (`C:\Users\Nome Cognome\...`), idempotently: registration runs on every `connect`/`sync` and never re-quotes what it already quoted. Bare names (e.g. `jq`) are left verbatim, resolved via PATH. An **absolute** command whose own path contains a space must arrive already quoted from the KB: unquoted, `C:\Program Files\x.exe --flag` is indistinguishable from the command `C:\Program` with arguments, so it is left as declared rather than guessed at. On Windows, a command bound for a POSIX shell — every Claude Code hook, which Claude Code runs through Git Bash, and the `sh -c` of a generated OpenCode plugin — has the backslashes of its executable turned into forward slashes (`C:/Users/user/.claude/hooks/…`): the shell would otherwise delete them and the hook would never run. Arguments are left verbatim. An entry written before this, with the backslash form, is still recognised as the hook's own and replaced by the next `sync`, not duplicated ([D267](decisions/D267-hook-commands-for-a-posix-shell-use-forward-slashes-on-windows.md)).
- <a id="executable-flag"></a>**Executable flag.** A file's executable bit comes from the KB and is preserved on materialization (including skill scripts); hooks retain an unconditional executable floor for every file other than `hook.json`, which is always non-executable. The effective mode is part of the versioned artifact hash, so `chmod` alone changes the revision and realigns existing installs. On Windows the filesystem has no execute permission (`internal/execbit`): the flag still travels in the manifest and is still stored in the KB, but a file read back from disk there never reports it, and a mode difference there is not drift — a KB script that round-trips through a Windows client's disk loses its bit ([D219](decisions/D219-windows-has-no-execute-bit-and-kb-paths-are-slash-paths.md)).

  This is why the flag is **lockfile state, not an observation**, where the filesystem has none. `materialized_hash` is recorded from what the writer intended, so on-disk verification must reproduce that same intent: it takes the flag from the lockfile's `executable_paths` instead of from the mode, falling back — for a lockfile written before that field, or one with nothing executable — to the kind's own rule, which is exact for a hook and means "not executable" for everything else. Without it a skill shipping an executable helper reported `modified` on every check, permanently, with no repair able to clear it. On unix nothing changes: there the filesystem is the authority, because an external `chmod` is real drift and reporting it is the point. The artifact hash itself is untouched by all of this — normalising the mode inside the digest would make every client's expanded hash differ from the server's `content_hash` and re-materialize every installation at once ([D216](decisions/D216-the-client-half-reaches-parity-on-windows.md)).
...
clean absolute path, in either platform's spelling — POSIX, drive-absolute (`C:\srv\srv.exe`, `C:/srv/srv.exe`)
or UNC (`\\host\share\srv.exe`) — because a descriptor lives in the KB and is validated by every client,
whichever wrote it: the verdict is the same on every host. Never a shell expression: Cartographer
passes command and arguments separately and never starts a shell. Shell metacharacters and NUL are
refused; parentheses are the one exception and only for an **absolute** command, so
`C:\Program Files (x86)\srv\srv.exe` is expressible — an absolute command is handed to `exec.Command`
with a separate argv and never reaches a shell, while a bare name has no reason to carry one
([D216](decisions/D216-the-client-half-reaches-parity-on-windows.md)). Arguments keep their order and `env` values are `${VAR}` references only, so
