...
266 records, one file each, grouped by the topic they belong to.
...
- [D219 — Windows has no execute bit, and a KB path is a slash path](decisions/D219-windows-has-no-execute-bit-and-kb-paths-are-slash-paths.md)
- [D233 — The Atlas wears the Cartographer brand](decisions/D233-the-atlas-wears-the-cartographer-brand.md)
- [D234 — The 3D atlas is a living elastic network](decisions/D234-the-3d-atlas-is-a-living-elastic-network.md)
- [D235 — The Atlas draws its graph only in 3D](decisions/D235-the-atlas-draws-only-in-3d.md)
- [D238 — The Atlas UI shows a KB's artifacts in their own panel](decisions/D238-atlas-artifacts-panel.md)
- [D239 — The reading panel is resized by a splitter over the graph](decisions/D239-resizable-inspector.md)
- [D240 — A selection hidden by a filter stays selected, the graph draws none](decisions/D240-filtered-out-selection.md)
...
- [D241 — The link graph is a stat-validated in-memory cache](decisions/D241-link-graph-stat-validated-cache.md)
- [D248 — Backlink rewrite follows the graph](decisions/D248-backlink-rewrite-follows-the-graph.md)
- [D269 — Reserve the services namespace and make concept moves namespace-aware](decisions/D269-services-namespace-consistency.md)
- [D270 — Assets above 10 MiB are not versioned, and the asset listing survives stray files](decisions/D270-assets-above-10-mib-are-not-versioned.md)
...
- [D226 — Typed read projections and a read-only UI API](decisions/D226-typed-graph-projections-and-ui-api.md)
- [D229 — A map contract can change after creation](decisions/D229-a-map-contract-can-change-after-creation.md)
- [D242 — Graph retrieval tools: graph_context, link_suggest, graph_path](decisions/D242-graph-retrieval-tools.md)
- [D243 — Structural lint and supersede relations](decisions/D243-structural-lint-and-supersede-relations.md)
- [D245 — Search indexes follow the files](decisions/D245-search-indexes-follow-the-files.md)
- [D246 — Search weights titles and folds diacritics](decisions/D246-search-weights-titles-and-folds-diacritics.md)
- [D247 — Search misses reveal knowledge gaps](decisions/D247-search-misses-reveal-knowledge-gaps.md)
- [D249 — graph_neighbors on the visible graph](decisions/D249-graph-neighbors-on-the-visible-graph.md)
- [D250 — changes_since reports link changes](decisions/D250-changes-since-reports-link-changes.md)
- [D251 — search uses centrality as a prior](decisions/D251-search-uses-centrality-as-a-prior.md)
...
- [D215 — The per-KB lock splits by platform, and Windows becomes a CI gate](decisions/D215-the-kb-lock-splits-by-platform-and-windows-is-a-gate.md)
- [D237 — A bounded fetch, and a read backoff, for a remote that does not answer](decisions/D237-bounded-fetch-and-read-backoff-for-an-unreachable-remote.md)
- [D258 — Reads refresh in the background, and never wait for the remote](decisions/D258-reads-refresh-in-the-background.md)
- [D264 — KB writes follow the remote default branch](decisions/D264-kb-writes-follow-the-remote-default-branch.md)
- [D265 — The author is the committer when git has none](decisions/D265-the-author-is-the-committer-when-git-has-none.md)
...
- [D260 — sops runs in a hermetic environment: allowlisted variables, an empty home, the per-KB key file](decisions/D260-sops-runs-in-a-hermetic-environment.md)
- [D261 — service_get redacts resolved secrets unless reveal, and the audit records reveal](decisions/D261-service-get-redacts-and-the-audit-records-reveal.md)
...
- [D216 — The client half reaches parity on Windows, and the executable flag becomes lockfile state](decisions/D216-the-client-half-reaches-parity-on-windows.md)
- [D230 — Codex hooks register in hooks.json](decisions/D230-codex-hooks-register-in-hooks-json.md)
- [D231 — Local clients get the loopback literal](decisions/D231-local-clients-get-the-loopback-literal.md)
- [D236 — A Codex hook registration left in config.toml is drift](decisions/D236-a-stale-codex-hook-registration-is-drift.md)
- [D256 — The client pulls the KBs' manifests concurrently, and reports errors in KB order](decisions/D256-concurrent-sync-pulls.md)
- [D257 — Sync continues past a KB that cannot be pulled](decisions/D257-sync-continues-past-a-kb-that-cannot-be-pulled.md)
- [D262 — Path placeholders are visible, fully resolved and onboarded](decisions/D262-path-placeholders-are-visible-resolved-and-onboarded.md)
- [D263 — A KB declares its path placeholder registry](decisions/D263-a-kb-declares-its-path-placeholder-registry.md)
- [D267 — Hook commands bound for a POSIX shell use forward slashes on Windows, and one that cannot run is drift](decisions/D267-hook-commands-for-a-posix-shell-use-forward-slashes-on-windows.md)
...
- [D220 — `sync` states the server change; only `status` and `doctor` recommend `reconnect`](decisions/D220-sync-states-the-server-change-without-recommending.md)
...
- [D254 — Agents are told when Cartographer is out of date](decisions/D254-agents-are-told-when-cartographer-is-out-of-date.md)
...
- [D217 — On Windows the native service is a per-user Scheduled Task, driven by PowerShell](decisions/D217-the-native-service-on-windows-is-a-per-user-scheduled-task.md)
- [D218 — winget is the only Windows channel, and its archive is a zip](decisions/D218-winget-is-the-only-windows-channel-and-the-archive-is-a-zip.md)
...
- [D255 — CI runs what the change touches](decisions/D255-ci-runs-what-the-change-touches.md)
- [D266 — A Windows restart waits for the old process to leave its port, and /health names the process](decisions/D266-a-windows-restart-waits-for-the-old-process-and-proves-the-new-one.md)
- [D268 — The generated config of a loopback service pins `auth.mode: "off"`, and `setup` stops on a 401 before `connect`](decisions/D268-the-local-service-pins-auth-off.md)
...
- [D228 — Browser-level verification of the Atlas UI](decisions/D228-browser-level-verification-of-the-atlas-ui.md)
- [D232 — Coordinate is the mark, Rilievo a motif](decisions/D232-coordinate-is-the-mark-rilievo-a-motif.md)
- [D259 — Published text carries no private names](decisions/D259-published-text-carries-no-private-names.md)
<!-- decisions:index:end -->
