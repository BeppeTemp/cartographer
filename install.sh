...
detect_target() {
    ...
    # install.ps1 instruction whatever `uname -m` says there.
    case "$os" in
        darwin|linux) ;;
        ...
        # (D252), for `update` and `uninstall` as well as `install`, so naming it
        # is the whole answer.
        mingw*|msys*|cygwin*|windows*)
            ...
            log ""
            log "update and uninstall: see the README (\"Install\")."
            exit 1
            ;;
