...
func toolSyncStatus(k *kb.KB) Tool {
	return Tool{Name: "sync_status", ReadOnly: true,
		...
		Handler: func(ctx requestContext, args json.RawMessage) (ToolResult, error) {
			...
			out, _ := json.MarshalIndent(map[string]any{"state": s.State, "last_error": s.LastError, "last_attempt_at": s.LastAttemptAt, "head_sha": s.HeadSHA, "unpushed_commits": s.UnpushedCommits, "identity_warning": s.IdentityWarning, "attempts": s.Attempts, "branch": s.Branch, "remote_default_branch": s.RemoteDefaultBranch, "push_mode": mode, "git_workflow": server.Profile, "profile": server.Profile, "base_branch": server.BaseBranch, "working_branch": server.WorkingBranch, "pr_number": server.PRNumber, "pr_url": server.PRURL, "pr_head_sha": server.PRHeadSHA, "last_forge_error": server.LastForgeError}, "", "  ")
