...
func TestRegisterTool_WithPrefix(t *testing.T) {
	...
	if _, ok := s.Tools()["engteam__atlas_overview"]; !ok {
		t.Fatalf("expected prefixed tool name %q to be registered; got %v", "engteam__atlas_overview", s.Tools())
	}
}
...
func TestServer_ToolRequiresWrite_Prefixed(t *testing.T) {
	...
	if s.ToolRequiresWrite("engteam__atlas_overview") {
		t.Error("engteam__atlas_overview (read-only tool, prefixed) misclassified as requiring write")
	}
	if !s.ToolRequiresWrite("engteam__concept_write") {
		t.Error("engteam__concept_write (write tool, prefixed) misclassified as read-only")
	}
	// Unknown names remain fail-closed (require write) whether or not they
	// carry the server's prefix.
	if !s.ToolRequiresWrite("engteam__no_such_tool") {
...
func TestServer_ToolsProfile_Prefixed(t *testing.T) {
	...
	if _, ok := names["engteam__validate"]; ok {
		t.Error("engteam__validate is an advanced tool and must be hidden under the agent profile, even prefixed")
	}
...
func TestMountKBWithPrefix_TwoKBs_DisjointNames(t *testing.T) {
	...
	if err := multi.MountKBWithPrefix("notes-kb", "", func(s *Server) {
		RegisterKBTools(s, kUnprefixed, Deps{})
	}); err != nil {
		t.Fatalf("MountKBWithPrefix(notes-kb): %v", err)
	}
	kPrefixed := setupTestKB(t)
	if err := multi.MountKBWithPrefix("eng-team", "engteam", func(s *Server) {
		RegisterKBTools(s, kPrefixed, Deps{})
	}); err != nil {
		t.Fatalf("MountKBWithPrefix(eng-team): %v", err)
	}
	...
	rrPersonal := doMCP(handler, "notes-kb", "", toolsListBody)
	if rrPersonal.Code != http.StatusOK {
		t.Fatalf("notes-kb tools/list: status = %d, body=%s", rrPersonal.Code, rrPersonal.Body.String())
	}
	...
	rrEngTeam := doMCP(handler, "eng-team", "", toolsListBody)
	if rrEngTeam.Code != http.StatusOK {
		t.Fatalf("eng-team tools/list: status = %d, body=%s", rrEngTeam.Code, rrEngTeam.Body.String())
	}
	var respEngTeam Response
	if err := json.Unmarshal(rrEngTeam.Body.Bytes(), &respEngTeam); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	engTeamNames := toolNamesFromToolsList(t, respEngTeam)

	if !personalNames["atlas_overview"] {
		t.Error("notes-kb should expose the bare atlas_overview")
	}
	if engTeamNames["atlas_overview"] {
		t.Error("eng-team must not expose the bare atlas_overview (it's prefixed)")
	}
	if !engTeamNames["engteam__atlas_overview"] {
		t.Error("eng-team should expose engteam__atlas_overview")
	}
	if personalNames["engteam__atlas_overview"] {
		t.Error("notes-kb must not expose the eng-team-prefixed name")
	}
	for name := range engTeamNames {
...
func TestMountKBWithPrefix_ScopedTokenMatrix(t *testing.T) {
	...
	ts := auth.NewScopedTokenStore([]auth.ScopedToken{
		{Token: "r-tok", Scopes: []auth.KBScope{{KB: "eng-team", Write: false}}},
		{Token: "rw-tok", Scopes: []auth.KBScope{{KB: "eng-team", Write: true}}},
	})
	handler := ts.Middleware(multi.Handler())

	rr := doMCP(handler, "eng-team", "r-tok", writeToolCallBody("engteam__concept_write"))
	assertMCPForbidden(t, rr)

	rr = doMCP(handler, "eng-team", "r-tok", writeToolCallBody("engteam__atlas_overview"))
	if rr.Code != http.StatusOK {
		t.Fatalf("prefixed read tool with r scope: status = %d, want 200; body=%s", rr.Code, rr.Body.String())
	}

	rr = doMCP(handler, "eng-team", "rw-tok", writeToolCallBody("engteam__concept_write"))
	if rr.Code != http.StatusOK {
	...
	rr = doMCP(handler, "eng-team", "r-tok", writeToolCallBody("engteam__index_patch"))
	assertMCPForbidden(t, rr)

	rr = doMCP(handler, "eng-team", "rw-tok", writeToolCallBody("engteam__index_patch"))
	if rr.Code != http.StatusOK {
