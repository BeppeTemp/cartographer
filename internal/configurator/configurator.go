...
// server over HTTP (see D37): there is no stdio transport option.
...
// Codex CLI never actually reads (it only reads config.toml); see D58.
...
// D69's note on internal/provisioning surfacing a Warning for
