...
func TestProjections_GlobalAndWorkspacesAreSeparate(t *testing.T) {
	...
	lf.SetProjection(Projection{Provider: "claude", Workspace: "/w/work"}, lockWith("work-skill/SKILL.md"), "git.example.com/team/work")
	...
	work := lf.ForProjection(Projection{Provider: "claude", Workspace: "/w/work"})
	if len(work.Managed) != 1 || work.Managed[0].Path != "work-skill/SKILL.md" {
		t.Fatalf("work projection = %+v", work.Managed)
	}
	...
	if work.BaseDir != "/w/work" {
		t.Errorf("workspace lock BaseDir = %q, want the workspace path", work.BaseDir)
	}
	if LockBaseDir(work, "/home/user") != "/w/work" {
	...
	if got := lf.WorkspaceRemote("/w/work"); got != "git.example.com/team/work" {
...
func TestLockFileRoundTrip_WorkspacesSurvive(t *testing.T) {
	...
	var lf LockFile
	lf.SetProvider("claude", lockWith("global/SKILL.md"))
	lf.SetProjection(Projection{Provider: "claude", Workspace: "/w/work"}, lockWith("work/SKILL.md"), "git.example.com/team/work")
	...
	if got := back.ForProjection(Projection{Provider: "claude", Workspace: "/w/work"}); len(got.Managed) != 1 || got.BaseDir != "/w/work" {
	...
	if back.WorkspaceRemote("/w/work") != "git.example.com/team/work" {
